...
| U4 | P3 | `phone/tools/specload.py` (stdlib YAML subset loader), `phone/tools/validate_spec.py`, `phone/tools/fit_check.py`, `phone/tests/test_tools.py` | tests pass; fit check reports every bay | U3 | done |
| U5 | P3 | `phone/spec/modules/{compute,display,battery,camera-rear,sensor-front,port,radio,frame}.yaml`, `phone/tools/validate_modules.py`, `phone/tests/test_modules.py` | validate + fit pass for all eight | U4 | done |
| U6 | P3 | `phone/tools/power_budget.py` (scenarios: idle, screen-on, video, 5G data), `phone/tools/bom_rollup.py`, `phone/tools/repairability.py`, `phone/tools/report.py` → `phone/build/*.md`, `phone/tests/test_reports.py` | reports generate with zero failures; BOM vs ceiling stated | U5 | done |
| U7 | P4 | `phone/hardware/cad/frame.py` (CadQuery from `device.yaml`), `modules.py` dummies, `exploded.py` SVG, `gen_cad.py` driver → `phone/build/*.step,*.stl,*.svg`; `phone/hardware/fasteners-and-tolerances.md` | STEP exports; no bay collision; assembly order listed | U5 | in progress — `modules.py` (layout+collision check) and `frame.py` (structural solid, volume/mass) done and verified; `exploded.py`, `gen_cad.py`, the fasteners doc and tests still pending, see `project/state.md` |

...
| D-008 | 2026-09-22 | SoC candidate for BC-1 v1: a Qualcomm QCM6490-class part, chosen for its upstream Linux support and its long industrial availability window; alternatives tracked in research | PROPOSED | Numbers flagged; source will be `phone/docs/research/01-soc-candidates.md` once U11 writes it — not yet created, see `project/plan.md` |
| D-009 | 2026-09-22 | Software: mainline-Linux-first (Debian/postmarketOS-class userland) with AOSP as a second supported image on the same kernel; no vendor kernel forks | PROPOSED | An OS the community can keep alive outlives any vendor; detail will be `phone/software/os-stack.md` once U9 writes it — not yet created, see `project/plan.md` |
...
| D-016 | 2026-09-22 | Fixed two `phone/spec/device.yaml` bugs `validate_spec.py` (U4) caught on first run: `port` bay's lane referenced `USB3` instead of the declared lane group id `USB3/PCIe`; `compute` bay's `lanes` list wrongly included `I2C:1`/`GPIO:1`, which `bus.md` defines as always-present pin groups, not per-bay high-speed lane entries | PROPOSED | Mechanical correction to match `phone/spec/bus.md`'s own definition of the `lanes` field; not a design change |
